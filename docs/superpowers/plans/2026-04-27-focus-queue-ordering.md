# Focus Queue Drag-to-Order Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the unordered Set-based task selection in Focus Mode with an ordered drag-to-reorder queue so users control which sticky note appears first.

**Architecture:** Single file change — `app/focus/page.tsx`. State changes from `Set<string>` to `string[]`. The "Your pile" panel gains a `@hello-pangea/dnd` drag list with position numbers and a remove button per item. The "Start Focus Session →" button moves from the task list panel to the queue panel.

**Tech Stack:** Next.js 16 App Router, `@hello-pangea/dnd` (already installed), lucide-react (`GripVertical`), Tailwind CSS
...
| File | Action |
|---|---|
| `app/focus/page.tsx` | Full rewrite — state, handlers, and queue panel JSX |

`components/StickyPile.tsx` — no changes. It already accepts `tasks: Task[]` in order.

---

## Task 1: Rewrite app/focus/page.tsx
...
Write the following to `app/focus/page.tsx`:
...
git commit -m "feat: drag-to-order queue in Focus Mode"
```
