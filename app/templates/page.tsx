...
export default function TemplatesPage() {
  ...
  const [editing, setEditing] = useState<Set<string>>(new Set()); // which templates are in edit mode
    ...
    <div className="space-y-6">
      ...
      {showNewForm && (
        <div className="rounded-xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 p-4 space-y-3">
          <h2 className="font-medium text-sm">New Template</h2>
          <Input
                      ...
                      {tasks.map((task, i) => (
                        <div key={i} className="grid grid-cols-12 gap-2 px-4 py-2 items-center">
                          ...
                          <div className="col-span-2">
                            <Select
                              value={String(task.sprint)}
                              onValueChange={(v) => updateTask(template.id, i, "sprint", parseInt(v))}
                            >
                          ...
                          <div className="col-span-2">
                            <Select
                              value={task.workCategory}
                              onValueChange={(v) => updateTask(template.id, i, "workCategory", v)}
                            >
                              <SelectTrigger className="h-7 text-xs"><SelectValue /></SelectTrigger>
                              <SelectContent>
